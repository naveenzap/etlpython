...
with open('Sales_data_sample.csv', 'r') as f:
    reader = csv.DictReader(f)
    print(reader.fieldnames)

    # Prepare the INSERT statement with column names
    column_names = ", ".join(reader.fieldnames)
    insert_statement = f"INSERT INTO sales ({column_names}) VALUES ({', '.join(['%s' for _ in reader.fieldnames])})"
    ...
    for row in reader:
        ...
        try:
            ...
            date = datetime.strptime(row['ORDERDATE'], '%d-%m-%Y').strftime('%Y-%m-%d')
            ...
            #date = datetime.strptime(row['orderdate'], '%d/%m/%Y').strftime('%Y-%m-%d')
        '''except ValueError:
            # Handle invalid date format or log the error
            date = datetime.strptime(row['orderdate'], '%m/%d/%Y').strftime('%Y-%m-%d')
            print(f"Error parsing date {row['orderdate']}")'''
        #row['orderdate'] = date3
        row['ORDERDATE'] = date
        #row['shipdate'] = date2
        ...
        '''cursor.copy_from(f, 'Sales_Records_1M', sep=',',
